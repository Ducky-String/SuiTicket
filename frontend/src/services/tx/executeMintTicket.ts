...
export const handleMintTicket = async (movieName: string, imageUrl: string, quantity: number) => {
  ...
  // 1. Tách đúng 0.01 SUI từ số dư hiện có của người dùng
  const [paymentCoin] = txb.splitCoins(txb.gas, [txb.pure.u64(10000000 * quantity)]);
  // 2. Thực hiện gọi hàm Move
  txb.moveCall({
    target: `${PACKAGE_ID}::suiticket::mint_ticket`,
    arguments: [
      paymentCoin, // Truyền đồng xu 0.01 SUI vào
